...
pub fn generate_wiggle_fill(
    ...
    if polygon.outer.len() > 6 {
        return generate_wiggle_fill_tessellated(polygon, spacing, angle_degrees, amplitude, frequency);
    }

    // For simple polygons, use the direct approach
    generate_wiggle_fill_direct(polygon, spacing, angle_degrees, amplitude, frequency)
}

/// Direct wiggle fill - generates all lines then clips.
/// Used for simple polygons where tessellation overhead isn't worth it.
fn generate_wiggle_fill_direct(
    polygon: &Polygon,
    spacing: f64,
    angle_degrees: f64,
    amplitude: f64,
...
/// Tessellation-based wiggle fill - much faster for complex polygons.
///
/// Breaks the polygon into triangles and fills each one separately.
/// The wave function uses global coordinates so waves align across triangles.
fn generate_wiggle_fill_tessellated(
    polygon: &Polygon,
    spacing: f64,
    angle_degrees: f64,
    amplitude: f64,
    ...
    for tri in &triangles {
        ...
        let num_segments = (tri_diagonal * 2.0 / segment_length) as i32 + 2;
        ...
        for i in -num_lines..=num_lines {
            ...
            for j in -num_segments/2..=num_segments/2 {
                let t1 = j as f64 * segment_length;
                let t2 = t1 + segment_length;

                // Sinusoidal displacement using global coordinates for phase
                let wave1 = amplitude * (t1 * frequency * 2.0 * PI).sin();
                let wave2 = amplitude * (t2 * frequency * 2.0 * PI).sin();
...
/// Returns the clipped line if any part is inside the triangle.
fn clip_line_to_triangle(
    x1: f64, y1: f64, x2: f64, y2: f64,
    edges: &[(Point, Point); 3],
) -> Option<Line> {
    // Check if both endpoints are inside triangle
    ...
    // Build the clipped line from endpoints and intersections
