#!/usr/bin/env python3
"""
Vermont Towns Random Fill Generator

Creates a plotter-ready SVG with each Vermont town filled with a
...
def main():
    ...
    if not input_svg.exists():
        ...
        print("Run: strata build examples/vt_towns_fill.strata.yaml")
        sys.exit(1)
    ...
    print(f"Found {len(paths)} towns to fill")

    # Set random seed for reproducibility (change for different results)
    random.seed(42)

    # Generate fills for each town
    all_elements = []
